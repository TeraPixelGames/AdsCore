...
func show(placement: String, ad_type: String, _opts: Dictionary = {}) -> void:
	...
	match ad_type:
		"interstitial":
			ok = _call_js("(function(){if(window.PokiSDK && PokiSDK.commercialBreak){PokiSDK.commercialBreak(); return true;} return false;})()")
		"rewarded":
			ok = _call_js("(function(){if(window.PokiSDK && PokiSDK.rewardedBreak){PokiSDK.rewardedBreak(); return true;} return false;})()")
		_:
			ok = false
	if not ok:
		emit_failed(placement, ad_type, Ads.ErrorCode.LOAD_FAILED_INTERNAL, "poki_js_unavailable")
		return
