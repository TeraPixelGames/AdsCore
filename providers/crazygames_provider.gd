...
func show(placement: String, ad_type: String, _opts: Dictionary = {}) -> void:
	...
	match ad_type:
		"interstitial":
			ok = _call_js("(function(){if(window.CrazyGames && CrazyGames.SDK && CrazyGames.SDK.showAd){CrazyGames.SDK.showAd('midgame'); return true;} return false;})()")
		"rewarded":
			ok = _call_js("(function(){if(window.CrazyGames && CrazyGames.SDK && CrazyGames.SDK.showAd){CrazyGames.SDK.showAd('rewarded'); return true;} return false;})()")
		"playable":
			ok = _call_js("(function(){if(window.CrazyGames && CrazyGames.SDK && CrazyGames.SDK.showAd){CrazyGames.SDK.showAd('playable'); return true;} return false;})()")
		_:
			ok = false
	if not ok:
		emit_failed(placement, ad_type, Ads.ErrorCode.LOAD_FAILED_INTERNAL, "crazygames_js_unavailable")
		return
