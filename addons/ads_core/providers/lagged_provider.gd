...
func show(placement: String, ad_type: String, _opts: Dictionary = {}) -> void:
	...
	match ad_type:
		"interstitial":
			ok = _call_js("(function(){if(window.laggedAPI && laggedAPI.ads){laggedAPI.ads.show(); return true;} return false;})()")
		"rewarded":
			ok = _call_js("(function(){if(window.laggedAPI && laggedAPI.rewards){laggedAPI.rewards.show(); return true;} return false;})()")
		_:
			ok = false
	if not ok:
		emit_failed(placement, ad_type, Ads.ErrorCode.LOAD_FAILED_INTERNAL, "lagged_js_unavailable")
		return
